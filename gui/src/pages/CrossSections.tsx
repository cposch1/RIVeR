...
export const CrossSections = () => {
  ...
  const handleDrop = ( e: React.DragEvent<HTMLDivElement> ) => {
    ...
    if (files.length > 0) {
      ...
      if ( type === "ipcam"){
        onGetBathimetry({ bathimetryPath: path, cameraMatrix: ipcam.cameraSolution?.cameraMatrix, zLimits: { min: bathimetry.yMin ?? 0, max: bathimetry.yMax ?? 0 }  })
          .then((error) => {
            if (error?.message) {
        ...
        onGetBathimetry({ bathimetryPath: path })
          .catch((error) => onSetErrorMessage(error.message));
      }
    }
  }
